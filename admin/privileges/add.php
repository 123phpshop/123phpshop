...
<body>
	<span class="phpshop123_title">添加权限</span>
	<div id="doc_help"
		style="display: inline; height: 40px; line-height: 50px; color: #CCCCCC;">
		<a style="color: #CCCCCC; margin-left: 3px;" target="_blank"
			href="<?php echo isset($doc_url)?"http://www.123phpshop.com/doc/v1.6/".$doc_url:"http://www.123phpshop.com/doc/";?>">[文档]</a><a
...
<?php if ($totalRows_getByName > 0) { // Show if recordset not empty ?>
  ...
  <form method="post" name="form1" id="form1"
		...
		<option
							value="<?=str_replace($_SERVER['DOCUMENT_ROOT'],'',$file_item);?>"><?=str_replace($_SERVER['DOCUMENT_ROOT'],'',$file_item);?></option>
		<?php }?>
      </select> <label> <input name="para" type="text" id="para" />
				</label></td>
			</tr>
			<tr valign="baseline">
				<td nowrap align="right">菜单:</td>
				<td><label> <input name="is_menu" type="checkbox" id="is_menu"
						value="checkbox" /> 如果想要将这个权限设置为左侧的菜单项，请勾选此项
				</label></td>
			</tr>
			<tr valign="baseline">
      ...
      <td><label>
		...
		<input type="hidden" name="parent_id" value=""> <input type="hidden"
	...
	<table width="100%" border="1" class="phpshop123_list_box">
     ...
     <?php do { ?>
      <tr>
			...
			<td><a href="remove.php?id=<?php echo $row_privileges['id']; ?>"
