...
<body>
	<span class="phpshop123_title">编辑权限</span>
	<div id="doc_help"
...
<?php if ($totalRows_getById == 0) { // Show if recordset empty ?>
  ...
  <form method="post" name="form1"
		...
		<table align="center" class="phpshop123_form_box">
			<tr valign="baseline">
				...
				<td><input type="text" name="name"
					value="<?php echo $row_getById['name']; ?>" size="32"></td>
			</tr>
			<tr valign="baseline">
 			...
 			<?php  foreach($global_file_list_array as $file_item){?>
				<option
							<?php if(str_replace($_SERVER['DOCUMENT_ROOT'],'',$file_item)==$row_getById['file_name']){ echo " selected ";}?>
							value="<?php echo str_replace($_SERVER['DOCUMENT_ROOT'],'',$file_item);?>"><?php echo str_replace($_SERVER['DOCUMENT_ROOT'],'',$file_item);?></option>
			<?php }?>
		</select> <input name="para" type="text" id="para"
					value="<?php echo $row_getById['para']; ?>" /></td>
			</tr>
			<tr valign="baseline">
				<td nowrap="nowrap" align="right">菜单:</td>
				<td><label> <input
						<?php if ($row_getById['is_menu']==1) {echo "checked=\"checked\"";} ?>
						name="is_menu" type="checkbox" id="is_menu" value="checkbox" />
						如果想要将这个权限设置为左侧的菜单项，请勾选此项
				</label></td>
			</tr>
			<tr valign="baseline">
				<td nowrap align="right" valign="top">排序：</td>
				<td><label> <input name="sort" type="text" id="sort"
						value="<?php echo $row_getById['sort']; ?>" />
				</label></td>
			</tr>
			<!--tr valign="baseline">
		...
		<input type="hidden" name="MM_update" value="form1"> <input
			type="hidden" name="id" value="<?php echo $row_getById['id']; ?>">
	
	</form>
  <?php } // Show if recordset not empty ?><script language="JavaScript"
