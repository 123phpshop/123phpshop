...
if ((isset ( $_POST ["MM_insert"] )) && ($_POST ["MM_insert"] == "order_add_product_form")) {
	...
	if ($totalRows_check_product == 0 && isset ( $_POST ['is_present'] )) {
		...
		$insertSQL = sprintf ( "INSERT INTO order_item (should_pay_price,order_id,product_id, quantity, attr_value, is_present) VALUES (%s,%s, %s,%s, %s, %s)", GetSQLValueString ( 0.00, "double" ), GetSQLValueString ( $colname_order, "int" ), GetSQLValueString ( $_POST ['product_id'], "int" ), GetSQLValueString ( $_POST ['quantity'], "int" ), GetSQLValueString ( $_POST ['attr_value'], "text" ), GetSQLValueString ( isset ( $_POST ['is_present'] ) ? "true" : "", "defined", "1", "0" ) );
	...
	// 然后插入到订单产品表格中
	$insertSQL = sprintf ( "INSERT INTO order_item (order_id,product_id, quantity, attr_value, is_present) VALUES (%s, %s,%s, %s, %s)", GetSQLValueString ( $colname_order, "int" ), GetSQLValueString ( $_POST ['product_id'], "int" ), GetSQLValueString ( $_POST ['quantity'], "int" ), GetSQLValueString ( $_POST ['attr_value'], "text" ), GetSQLValueString ( isset ( $_POST ['is_present'] ) ? "true" : "", "defined", "1", "0" ) );
