...
$query_order = sprintf("SELECT * FROM orders WHERE id = %s AND order_status=".ORDER_STATUS_RETURNED_APPLIED." OR order_status=".ORDER_STATUS_REFUND, $colname_order);
...
if($could_delete==1){
	...
	$update_catalog = sprintf("update `orders` set order_status=".ORDER_STATUS_RETURNED." where id = %s", $colname_order);
	...
	if(!$update_catalog_query){
		...
		$order_log_sql="insert into order_log(order_id,message)values('".$colname_order."','".商家已经退货."')";
		$order=mysqli_query($localhost,$order_log_sql);
