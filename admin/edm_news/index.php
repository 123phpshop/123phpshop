...
// 处理批量操作
 if ((isset($_POST["form_op"])) && ($_POST["form_op"] == "batch_op") && count($_POST['news_id'])>0 ) {
	...
	switch($_POST['op_id']){
		case "100":
			$sql="update `edm_news` set is_delete=1 where id in (".implode(",",$_POST['news_id']).")";
		break;
		
		case "200":
			$sql="update `edm_news` set is_published=1 where id in (".implode(",",$_POST['news_id']).")";
		break;
		
		case "300":
			$sql="update `edm_news` set is_published=0 where id in (".implode(",",$_POST['news_id']).")";
		break;
	}
...
<body>
<span class="phpshop123_title">杂志列表</span>
...
<a href="add.php">
<input style="float:right;" type="submit" name="Submit2" value="添加杂志" />
</a>
