...
<body>
  ...
  <span class="phpshop123_title">用户搜索</span>
	...
	<a href="add.php"><input style="float: right;" type="submit"
		name="Submit2" value="添加用户" /> </a>

	<form id="user_search_form" name="user_search_form" method="get"
		action="">
	...
	<form id="form1" name="form1" method="post" action="">
		...
		<table width="100%" border="1" align="center"
			...
			<tr>
				<th width="4%"><label> <input type="checkbox" id="select_all"
						onClick="select_all_item()" />
				</label></th>
				<th width="34%">账号</th>
				<th width="21%">邮箱</th>
				<th width="23%">手机</th>
    ...
    <?php do {?>
      <tr>
				<td><label>
						<div align="center">
							<input name="user_id[]" type="checkbox" class="item_checkbox"
								id="user_id[]" value="<?php echo $row_users['id']; ?>" />
						</div>
				</label></td>
				<td><a href="detail.php?recordID=<?php echo $row_users['id']; ?>"> <?php echo $row_users['username']; ?> </a>
				</td>
				<td><?php echo $row_users['email']; ?></td>
				<td><?php echo $row_users['mobile']; ?></td>
				<td><div align="right">
						<a onclick="return confirm('您确实要删除这条记录吗？')"
							href="remove.php?id=<?php echo $row_users['id']; ?>">删除</a> <a
      ...
      <?php } while ($row_users = mysqli_fetch_assoc($users));?>
		...
		<table width="200" border="0" class="phpshop123_infobox">
			<tr>
				<td width="5%"><label> <select name="op_id" id="op_id">
							<option value="0">请选择操作..</option>
							<option value="100">删除用户</option>
					</select>
				</label></td>
				<td width="95%"><label> <input type="submit" name="Submit3"
						value="确定" /> <input type="hidden" value="batch_op" name="form_op" />
				</label></td>
			</tr>
		</table>
		<br>
  ...
  <?php if ($totalRows_users == 0) { // Show if recordset not empty ?>
	...
	<div class="phpshop123_infobox">
		没有记录，马上<a href="add.php">添加用户</a>吧！
	</div>
    <?php } // Show if recordset not empty ?>
