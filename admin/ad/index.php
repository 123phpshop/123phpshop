...
if (! empty ( $_SERVER ['QUERY_STRING'] )) {
	...
	foreach ( $params as $param ) {
		if (stristr ( $param, "pageNum_ads" ) == false && stristr ( $param, "totalRows_ads" ) == false) {
...
<body>
	<span class="phpshop123_title">广告列表</span>
	<div id="doc_help"
  ...
  <p>
    ...
    <?php do { ?>
      <tr>
				...
				<td><a href="detail.php?recordID=<?php echo $row_ads['id']; ?>"> <?php echo $row_ads['name']; ?>&nbsp; </a>
				</td>
				<td><?php echo $row_ads['intro']; ?>&nbsp; </td>
				<td><?php echo $row_ads['create_time']; ?></td>
				<td><div align="right">
						<a onClick="return confirm('你确实要删除这条广告吗？')"
							href="remove.php?id=<?php echo $row_ads['id']; ?>">删除</a> <a
							href="update.php?id=<?php echo $row_ads['id']; ?>">更新</a> <a
      ...
      <?php } while ($row_ads = mysqli_fetch_assoc($ads)); ?>
		...
		<br>
            ...
            <a
						href="<?php printf("%s?pageNum_ads=%d%s", $currentPage, 0, $queryString_ads); ?>"
						class="phpshop123_paging">第一页</a>
            <?php } // Show if not first page ?>      </td>
					<td width="31%" align="center"><?php if ($pageNum_ads > 0) { // Show if not first page ?>
            <a
						href="<?php printf("%s?pageNum_ads=%d%s", $currentPage, max(0, $pageNum_ads - 1), $queryString_ads); ?>"
						class="phpshop123_paging">前一页</a>
            <?php } // Show if not first page ?>      </td>
					<td width="23%" align="center"><?php if ($pageNum_ads < $totalPages_ads) { // Show if not last page ?>
            <a
						href="<?php printf("%s?pageNum_ads=%d%s", $currentPage, min($totalPages_ads, $pageNum_ads + 1), $queryString_ads); ?>"
						class="phpshop123_paging">下一页</a>
            <?php } // Show if not last page ?>      </td>
					<td width="23%" align="center"><?php if ($pageNum_ads < $totalPages_ads) { // Show if not last page ?>
            <a
						href="<?php printf("%s?pageNum_ads=%d%s", $currentPage, $totalPages_ads, $queryString_ads); ?>"
